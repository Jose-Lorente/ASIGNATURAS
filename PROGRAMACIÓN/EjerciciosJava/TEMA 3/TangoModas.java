...
public class TangoModas {
    public static void main(String[] args) {
        ...
        System.out.printf("%-13s %10s€%n", codigoArticulo, df.format(precio));

        // Unidades vendidas y subtotal 
        System.out.printf("%-13s %10s€%n", unidades + " unidades", df.format(subtotal));

        // Descuento y descuento
        System.out.printf("%-13s %10s€%n", "-" + (int)descuentoPorcentaje + "%", "-" + df.format(descuento));

        // IVA y su monto alineados
        System.out.printf("%-13s %10s€%n", "16% IVA", "+" + df.format(iva));

        // Total en negrita con fondo gris
        System.out.printf("\033[1m%-13s \033[47m%10s€\033[0m%n", "TOTAL", df.format(total));
